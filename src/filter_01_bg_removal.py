...
def apply(image):
    ...
    try:
        ...
        # ✅ Fallback for CI / no-internet / model download failure
        h, w = image.shape[:2]
        fg_mask = np.full((h, w), 255, dtype=np.uint8)  # all-foreground mask
        print(f"[WARN] BG removal failed, using fallback: {e}")
